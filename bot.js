  ...
  if (!msg.guild) {
    ...
    if (!ehengel[msg.guild.id]) {
      ...
      if (ehengel[msg.guild.id].sistem == false) {
        ...
        if (msg.member.roles.find("name", "ᚖ Owner Of İmportal")) {
        } else {
...
client.on("message", async message => {
    if (message.member.roles.find("name", "ᚖ Owner Of İmportal")) return;
...
let limit = "10" // 3 kısmına ban limitinin kaç olmasını istiyorsanız yazınız.
  if(banlar >= limit) {
