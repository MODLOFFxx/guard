...
module.exports.run = (client, message, args) => {
    ...
    if (!message.member.roles.find("name", "ᚖ | Voice Mute Hammer")) {
        return message.channel.send(' **Bu Komutu Kullanmak için** \*`ᚖ | Voice Mute Hammer*\` **Rolüne Sahip Olman Lazım** ')
...
let kullanici = message.mentions.members.first() || message.guild.members.get(args[0])
  ...
  if (kullanici.id === message.author.id) return message.channel.send("Kendinizi susturamazsınız.");
    ...
    let süre = args[1]
  if(!süre) return message.channel.send("Lütfen doğru bir zaman dilimi giriniz. Örneğin: ***!sesli-sustur @kişi 1s/m/h/d sebep**");
  let sebep = args[2]
  if (!sebep) return message.channel.send("Lütfen bir sebep giriniz. Örneğin: ***!sesli-sustur @kişi 1s/m/h/d sebep**");
